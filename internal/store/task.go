...
func (s *pgStore) CreateTask(ctx context.Context, t Task) error {
	...
	_, err := s.pool.Exec(ctx,
		`INSERT INTO ingest_tasks (id, kb_id, document_id, status, retry_count, error_message, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		t.ID, t.KBID, t.DocumentID, t.Status, t.RetryCount, t.ErrorMessage, time.Now(), time.Now(),
	)
	return err
}
...
func (s *pgStore) GetTask(ctx context.Context, id string) (*Task, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT id, kb_id, document_id, status, retry_count, error_message, created_at, updated_at
		 FROM ingest_tasks WHERE id = $1`, id,
	).Scan(&t.ID, &t.KBID, &t.DocumentID, &t.Status, &t.RetryCount, &t.ErrorMessage, &t.CreatedAt, &t.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &t, nil
...
func (s *pgStore) UpdateTask(ctx context.Context, t Task) error {
	_, err := s.pool.Exec(ctx,
		`UPDATE ingest_tasks SET status = $2, retry_count = $3, error_message = $4, updated_at = $5 WHERE id = $1`,
		t.ID, t.Status, t.RetryCount, t.ErrorMessage, time.Now(),
	)
	return err
}
...
func (s *pgStore) ClaimPendingTasks(ctx context.Context, limit int) ([]Task, error) {
	rows, err := s.pool.Query(ctx,
		`UPDATE ingest_tasks SET status = 'processing', updated_at = now()
		 ...
		 RETURNING id, kb_id, document_id, status, retry_count, error_message, created_at, updated_at`,
		limit)
	if err != nil {
		return nil, err
	}
