...
# 注意：必须复制到 ./frontend/ 子目录——若用 `COPY frontend/ ./` 会把 frontend
# 内容散落到 /app 根目录并覆盖根 package.json（变成 binrag-frontend），导致
# pnpm --filter 把 root 也纳入 scope 执行 build，root 上下文缺 vue-tsc 而报
# spawn ENOENT（vue-tsc: not found）。
